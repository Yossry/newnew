odoo.define('flexipharmacy_ee.CreateRefillMedicinePopup', function(require) {
    'use strict';
    ...
    class CreateRefillMedicinePopup extends AbstractAwaitablePopup {
        ...
        async onInputKeyDownNumberVlidation(e) {
            if(e.which == 190 && e.which != 110 && e.which != 8 && e.which != 0 && e.key != this.env.pos.db.decimalSeparator() && e.key != this.env.pos.db.decimalSeparator() && (e.which < 48 || e.which > 57 || e.shiftKey) && (e.which < 96 || e.which > 105) && (e.which < 37 || e.which > 40)) {
                e.preventDefault();
            }
        }
        changeCustomer(){
        ...
        changeExecutionDate(event){
            this.state.ExecutionDate = moment(moment(new Date())).add(event, 'days').locale('en').format("DD/MM/YYYY")
        }
