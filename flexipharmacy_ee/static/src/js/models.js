odoo.define('flexipharmacy_ee.models', function (require) {
    ...
    models.load_fields("res.partner", ['remaining_wallet_amount','is_doctor']);
...
//                selected_doctor : false,
        ...
        get_won_points: function(){
            ...
            for (var line of this.get_orderlines()){
                if(line && line.product && line.product.id && this.pos.config.gift_card_product_id){
                    if(line.product.id == this.pos.config.gift_card_product_id[0]){
                        return
                    }
                }
                if(line && line.product && line.product.id && this.pos.config.wallet_product){
                    if(line.product.id == this.pos.config.wallet_product[0]){
                        return
                    }
                }
