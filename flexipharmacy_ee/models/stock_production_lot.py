...
class StockProductionLot(models.Model):
    ...
    def product_state_check(self):
        stock_production_lot_data = self.env['stock.production.lot'].search([('product_qty', '>', 0)])
        today_date = date.today()
        for each_stock_lot in stock_production_lot_data.filtered(lambda l: l.expiration_date):
