...
class AsplGiftCard(models.Model):
    ...
    def _send_mail_balance_and_expired_coupon(self, expired=False, balance=False):
        if expired:
            ...
            expired_template_id = self.env.ref('flexipharmacy_ee.email_template_for_coupon_expire_7')
        ...
        if balance:
            balance_template_id = self.env.ref('flexipharmacy_ee.email_template_regarding_balance')
    ...
    def write_gift_card_from_ui(self, new_card_no):
        ...
        try:
            template_obj = self.env.ref("flexipharmacy_ee.email_template_exchange_number")
...
class AsplGiftCardUse(models.Model):
    ...
    def create(self, vals):
        ...
        if res.pos_order_id:
            try:
                template_obj = self.env.ref('flexipharmacy_ee.email_template_regarding_card_use')
